...
const details = [
  ...
  {
    image: <IconBrandGithub color='#c4881c' size={16} stroke={2} />,
  ...
  return (
    <div className='flex w-full  justify-between flex-row items-center'>
      <div className='flex flex-col gap-2'>
        <h1 className='text-res_primary font-medium uppercase text-5xl'>
          {personal.name}
        </h1>
          ...
          {details.map((item) => (
            <div key={item.text} className='flex  gap-1 items-center '>
              ...
              {item.text.includes("https") ? (
                <Link
                  href={item.text}
                  className='text-dark_brown underline font-medium'
                >
                  {item.text}
                </Link>
              ) : (
                <p className='text-res_primary font-medium'>{item.text}</p>
              )}
            </div>
          ))}
        </div>
      </div>
      <div className='relative w-[180px] border-2 border-dotted border-opacity-50 border-dark_brown rounded-full h-[180px] '>
        <Image
          src={"/ovd.png"}
          alt='ovd'
          fill
